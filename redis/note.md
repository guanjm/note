...
>       - 弱一致性：备机节点异步复制主机节点数据。主机节点向备机发送同步数据时，备机挂机了，导致复制失败丢失数据。（redis）
...
> - 击穿
>   - 产生场景
>       - 基于高并发（访问量少的话，在缓存过期后访问会及时补充缓存，后续的访问也会访问到缓存）
>       - 基于原有缓存过期，着重点在于大量的请求访问到同一个过期key（key设置的存活时长或者是淘汰策略的key删除）
>   - 方案
>       - 问题点
>           - 无法避免缓存过期（受内存大小限制，缓存肯定有过期的一天，对缓存过期进行处理（延迟存活时间）不能解决问题）。
>           - 大量的请求（大量的请求正好命中过期的缓存，也许正好都访问是某一个key，这个key的请求直接访问到下一层（提示：下一个逻辑分支））
>       - 优化逻辑
>           1. 先查缓存
>               - 有
>                   - 直接返回缓存
...
>           1. 加锁成功后，程序挂掉了=>死锁
>               - 锁加过期时间
>           2. 加锁成功后，写入缓存前，处理逻辑耗时，超过锁的过期时间，其他请求又能继续加锁=>重复上锁
>               - 多线程，一个线程访问数据库，写入缓存，一个线程定时检测缓存是否有值更新锁的过期时间
> - 穿透
>   - 产生场景
>       - 基于数据库没有目标数据（数据库没有数据，导致无法缓存数据，请求直接访问数据库）
>   - 方案
>       - 问题点
>           - 数据库没有数据（无法预先缓存null）
>       - 优化逻辑
>           - bloomFilter（不能删除）、countingBloomFilter、cuckooFilter
>
> - 雪崩
>   - 产生场景
>       - 基于高并发
>       - 基于原有缓存过期，着重点在于大量的请求访问到一批同时过期key
>   - 方案
>       - 问题点
>           1. 缓存同时过期
>           2. 业务需要某时间节点更新缓存数据
...
> - 分布式锁
>   - redis分布式锁
>       1. set EX [seconds] NX 锁+过期时间
>       2. 多线程延迟过期时间
>       3. redisson现成的分布式锁工具
>   - redis和zookeeper
>       - 使用分布式锁时一般不关注性能效率了，更加关注准确性
>       - redis：快，实现复杂
>       - zookeeper：较慢，实现简单
>       
